"""Low-level OpenGL robot canvas, camera, picking, and gizmo interaction.
...
from gui.trajectory_colors import gl_color_for_frame
from core.trajectory.model import rpy_to_quat
from gui.transform_gizmo import GizmoInteractionState, TransformGizmo
...
class RobotCanvas3D(QOpenGLWidget):
    ...
    def render_geom_ids(model):
        return RobotRenderer.render_geom_ids(model)
    ...
    def _transform_matrix(position, rotation):
        return RobotRenderer.transform_matrix(position, rotation)
    ...
    def _apply_geom_material(model, geom_id):
        RobotRenderer.apply_geom_material(model, geom_id)
    ...
    def _begin_transparent_pass():
        RobotRenderer.begin_transparent_pass()

    @staticmethod
    def _end_transparent_pass():
        RobotRenderer.end_transparent_pass()
