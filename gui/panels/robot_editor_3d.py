"""High-level 3D robot editor panel composed around the OpenGL canvas."""
...
from core.ik.collision import CollisionAwareIKSolver, CollisionChecker
from gui.viewers.robot_canvas_3d import RobotCanvas3D
from gui.collapsible_sidebar import CollapsibleSidebar
from core.ik.tasks import (
    FootLockTask,
    JointRegularizationTask,
    PostureTask,
    RootPoseTask,
)
from gui.panels.ik_controls import IKControlPanel
from gui.widgets.joint_controls import JointControl
...
class RobotViewer3D(QWidget):
    ...
    def __init__(self, robot_model=None, error=None):
        ...
        self.preview_controller = PreviewTimelineController(robot_model)
        self.committed_state = self.preview_controller.committed_state
        self.preview_state = self.preview_controller.preview_state
        ...
        self.state_timeline = self.preview_controller.timeline
    ...
    def preview_active(self):
        return self.preview_controller.preview_active

    @preview_active.setter
    def preview_active(self, value):
        self.preview_controller.preview_active = bool(value)

    @property
    def current_time(self):
        return self.preview_controller.current_time

    @current_time.setter
    def current_time(self, value):
        self.preview_controller.current_time = float(value)
    ...
    def _build_ik_settings_widget(self):
        panel = IKControlPanel(self.robot_model)
        panel.influence_changed.connect(self._ik_influence_changed)
        panel.preset_requested.connect(self.apply_ik_preset)
        self.ik_control_panel = panel
        self.ik_damping = panel.damping
        self.ik_max_iterations = panel.max_iterations
        self.ik_step_size = panel.step_size
        self.ik_max_step = panel.max_step
        self.ik_position_tolerance = panel.position_tolerance
        self.ik_orientation_tolerance = panel.orientation_tolerance
        self.ik_task_controls = panel.task_controls
        self.ik_preset_box = panel.preset_box
        self.ik_influence_controls = panel.influence_controls
        return panel
    ...
    def begin_preview(self):
        if not self.preview_controller.begin_preview():
            return
    ...
    def accept_preview(self):
        if not self.preview_active:
            ...
            return
        self.preview_controller.accept_preview()
    ...
    def cancel_preview(self):
        if not self.robot_state:
            return
        self.preview_controller.cancel_preview()
        self.canvas.set_preview_visible(False)
        self._sync_joint_controls()
        self._set_target_to_selected_pose()
