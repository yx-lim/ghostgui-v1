...
from .csv_io import export_configurations_csv
...
class G1AnalyticFallbackBackend:
    def __init__(self):
    ...
    def export_last_solution_csv(self, csv_path):
        export_configurations_csv(
            self.last_solution, self.joint_names, csv_path
        )


class MujocoIKBackend(G1AnalyticFallbackBackend):
    """
    ...
    def __init__(self, model_path=MODEL_PATH, mj_model=None, adapter=None):
        ...
        self.state = adapter.create_state() if adapter is not None else None
        self.data = self.state.mj_data if self.state is not None else mujoco.MjData(self.model)
    ...
    def active_ik_tasks(self, active_targets):
        ...
        for frame_name, target in active_targets.items():
            ...
            kind, object_id, object_name, weight = binding
            if (
                self.adapter is not None
                and kind == "body"
                and object_name == self.adapter.root_body
            ):
                continue
            desired = np.array([target.x, target.y, target.z], dtype=float)
            tasks.append(
                (frame_name, kind, object_id, object_name, weight, desired)
            )
    ...
    def solve_position_ik(
        ...
        if self.state is not None:
            result = self.state.solve_weighted_tasks(
                [
                    TCPPositionTask(
                        name=f"Trajectory target {frame_name}",
                        weight=weight,
                        priority=2,
                        required=True,
                        tolerance=tolerance,
                    ...
                    for (
                        frame_name, kind, _, object_name, weight, desired
                    ) in tasks
                ],
            ...
            return result.error, result.success, result.iterations
        ...
        for iteration in range(max_iterations):
            ...
            for _, kind, object_id, _, weight, desired in tasks:
...
class BackendInterface:
    def __init__(self, mj_model=None, adapter=None):
        self.grouped_fallback_backend = G1AnalyticFallbackBackend()
        ...
        if MUJOCO_IK_AVAILABLE:
            try:
                ...
                self.backend = G1AnalyticFallbackBackend()
                self.using_cpp_backend = False
...
# Source compatibility for extensions that imported the former generic name.
PythonTrajectoryBackend = G1AnalyticFallbackBackend
