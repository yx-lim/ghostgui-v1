...
class Trajectory:
    def __init__(self, track_names=None):
        # A trajectory is no longer one mixed path. It is multiple named
        # timelines, one per robot frame/body part.
        names = LEGACY_DEFAULT_TRACK_NAMES if track_names is None else track_names
        self.tracks = {name: [] for name in names}
